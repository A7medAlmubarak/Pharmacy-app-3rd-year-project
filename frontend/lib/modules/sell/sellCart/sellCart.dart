...
import 'package:pharma_project/modules/sell/sellCart/sellcart_controller.dart';
...
class SellCart extends StatefulWidget {
  @override
  State<SellCart> createState() => _SellCartState();
}

class _SellCartState extends State<SellCart> {
  SellCartController controller = Get.find();
  ...
  void done() async {
    for (product test in controller.p) {
      controller.med.add(
          Med(drugId: test.id, sellItemQuantity: test.quantity.toString()));
    }
    print(controller.med.length);
    print(controller.med);
    for (Med test in controller.med) {
       print(test.drugId.toString() +"    "+ test.sellItemQuantity);
    }
